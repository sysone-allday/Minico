/*
SignUpController 클래스는 회원가입 화면에서 사용자의 입력값(ID, 비밀번호, 이메일 등)을 검증하고,
중복 ID 확인, 입력 형식 검사, 비밀번호 확인 등의 로직을 처리합니다.
모든 조건이 충족되면 Member 객체를 생성하여 캐릭터 선택 화면으로 전달하고,
가입 완료를 위한 다음 단계로 화면을 전환합니다.
배경 음악 재생, 로그인 화면 복귀 기능도 포함됩니다.
 */
